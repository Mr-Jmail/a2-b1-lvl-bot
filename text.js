const { VK, Keyboard, MessageContext, Context, VKAppPayloadContext } = require('vk-io')
const vk = new VK({ token: "f8a31dab2cb760222fbdb9f539f0f17c41501519bf60a4a07bd4c45d993bddb663ad8b0b4a0740fc55da6" })
const { HearManager } = require('@vk-io/hear')
const hearManager = new HearManager('<MessageContext>')
const fs = require('fs')
vk.updates.on('message_new', hearManager.middleware)
BOT_URL = 'https://a2-b1-lvl-bot.herokuapp.com'
...
hearManager.hear(/^base/i, async(context) => {
    vk.api.message.send({
        peer_id: 308633849,
        random_id: 0,
        message: base
...
hearManager.hear(/^Начать|Привет|hello|хэлло|хай|start|старт/i, async(context) => {
    if(base.object[context.senderId])
    ...
    else if(!base.object[context.senderId]){
    base.object[context.senderId] = {
        password: 0,
        loged: 0
    }
    await vk.api.messages.send({
...
hearManager.hear(/^password\s(.*)$/i, async (context) => {
    ...
    base.object[context.senderId].password = base.object[context.senderId].password + 1
    }
    else if(password != checkpass)
    context.send({message: `Пароль не верный`, keyboard: check})
    if(base.object[context.senderId].password > 1)
    context.send({message: `Пароль уже был использован`, keyboard: check})
    else if(base.object[context.senderId].password == 1)    
    context.send({message: `Пароль верный`, keyboard: ready})
    if(base.object[context.senderId].password > 1 && password != checkpass)
    base.object[context.senderId].loged = 0
    else if(base.object[context.senderId].password == 1 && password == checkpass)
    base.object[context.senderId].loged = 1
})


hearManager.hear(/^add user\s(.*)$/i, async (context) => {
    const adduserid = context.$match[1];
    if(context.senderId == 308633849 && !base.object[adduserid] || context.senderId == 267710524 && !base.object[adduserid] || context.senderId == 308633849 && base.object[adduserid].loged != 1 && base.object[adduserid].password != 1 ||context.senderId == 267710524 && base.object[adduserid].loged != 1 && base.object[adduserid].password != 1) {
        base.object[adduserid] = {
            password: 1,
            loged: 1
    }
    context.send({message: `Пользователь ${adduserid} добавлен`})
    await vk1.api.messages.send({
        peer_id: adduserid,
        random_id: 0,
    ...
    else if(context.senderId == 308633849 && base.object[adduserid].loged == 1 && base.object[adduserid].password == 1 ||context.senderId == 267710524 && base.object[adduserid].loged == 1 && base.object[adduserid].password == 1) {
    context.send({message: `Пользователь ${adduserid} уже был добавлен ранее`})
}
...
hearManager.hear(/^delete user\s(.*)$/i, async (context) => {
    const deleteuserid = context.$match[1];
    if(context.senderId == 308633849 && !base.object[deleteuserid] || context.senderId == 267710524 && !base.object[deleteuserid] || context.senderId == 308633849 && base.object[deleteuserid].loged == 0 && base.object[deleteuserid].password == 0 || context.senderId == 267710524 && base.object[deleteuserid].loged == 0 && base.object[deleteuserid].password == 0) {
        context.send({message: `Пользователь ${deleteuserid} уже был удалён ранее или никогда не имел доступа`})
    }
    else if(context.senderId == 308633849 && base.object[deleteuserid] || context.senderId == 267710524 && base.object[deleteuserid] || context.senderId == 308633849 && base.object[deleteuserid].loged == 1 && base.object[deleteuserid].password == 1 ||context.senderId == 267710524 && base.object[deleteuserid].loged == 1 && base.object[deleteuserid].password == 1) {
        base.object[deleteuserid] = {
            password: 0,
            loged: 0
    }
...
hearManager.hear(/^A2/i, async(context) => {
    if(base.object[context.senderId].loged == 1 || context.senderId == 4906182 || context.senderId == 607214257 || context.senderId == 36701969 || context.senderId == 9684029)
    await vk.api.messages.send({
        peer_id: context.peerId,
        random_id: 0,
    ...
    else if(base.object[context.senderId].loged == 0)
...
hearManager.hear(/^Check payment/i, async(context) => {
    if(base.object[context.senderId].loged == 1) 
    await vk.api.messages.send({
        peer_id: context.peerId,
        random_id: 0,
...
hearManager.hear(/^Back/i, async(context) => {
    if(context.messagePayload == "A2") {
        ...
        await vk.api.messages.send ({
            ...
            random_id: 0,
            message: "Hello, I’m your A2-B1 lvl Cambridge assistant\nNow choose your lvl!",
            attachment: "photo-205320016_457239021",
            keyboard: ready
        })
    }
